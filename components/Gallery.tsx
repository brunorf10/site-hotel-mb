          ...
          {visibleItems.map((item, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-primary/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center p-4 text-center">
                ...
                <span className="text-accent text-xs mt-1 uppercase tracking-widest">
                  {item.category}
                </span>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {selectedImage && (
          <>
            ...
            >
              <div className="relative max-w-6xl w-full h-full flex items-center justify-center pointer-events-auto">
                {/* Close Button - Fixed position in viewport */}
                <button
                  onClick={() => setSelectedImage(null)}
                  className="fixed top-4 right-4 md:top-6 md:right-6 text-white/80 hover:text-white transition-colors z-[100] bg-black/50 rounded-full p-2"
                ...
                <div className="relative">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 md:p-6 rounded-b-lg">
                    ...
                    <span className="text-accent text-xs md:text-sm uppercase tracking-widest">
                      {selectedImage.category}
                    </span>
                    <span className="text-white/60 text-xs md:text-sm ml-4">
                      {currentIndex + 1} / {filteredItems.length}
                    </span>
                  </div>
                  ...
                  {filteredItems.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white p-2 md:p-3 rounded-full transition-all backdrop-blur-sm"
                      ...
                      <button
                        ...
                        className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white p-2 md:p-3 rounded-full transition-all backdrop-blur-sm"
